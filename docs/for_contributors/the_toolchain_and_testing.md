...
The compiler tests push the source code through and generate and loads a `BEAM` file directly into the runtime and then make normal `Erlang` calls to that loaded module.
...
This is what a generated Eunit test suite looks like - the same inputs and outputs striped across 6 execution paths:
...
The generated Common Tests are recognisably the same tests - it is just the invocation is different.
