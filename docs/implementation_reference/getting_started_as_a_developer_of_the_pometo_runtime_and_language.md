...
%%% The first failing test you should fix is the bottom one - the simplest one-module(getting_started_as_a_developer_of_the_pometo_runtime_and_language_tests).
...
how_to_write_docs_pages_as_tests_1_compiler_test_() ->
    ...
    % ?debugFmt(" in how_to_write_docs_pages_as_tests_1_compiler~nCode:~n~ts~nExp:~n~ts~nGot:~n~ts~n", [Code, Expected, Got]),
...
how_to_write_docs_pages_as_tests_1_interpreter_test_() ->
    ...
    % ?debugFmt(" in how_to_write_docs_pages_as_tests_1_interpreter~nCode:~n~ts~nExp:~n~ts~nGot:~n~ts~n", [Code, Expected, Got]),
    ?_assertEqual(Expected, Got).
```
