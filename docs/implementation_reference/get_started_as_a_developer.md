...
-module(get_started_as_a_developer_tests).
...
how_to_write_docs_pages_as_tests_1_interpreter_test_() ->
    ...
    % ?debugFmt(" in how_to_write_docs_pages_as_tests_1_interpreter~nExp:~n~ts~nGot:~n~ts~n", [string:trim(Expected), string:trim(Got)]),
...
how_to_write_docs_pages_as_tests_1_compiler_test_() ->
    ...
    % ?debugFmt(" in how_to_write_docs_pages_as_tests_1_compiler~nExp:~n~ts~nGot:~n~ts~n", [string:trim(Expected), string:trim(Got)]),
    ?_assertEqual(string:trim(Expected), string:trim(Got)).
```
