...
basic_parser_failure_test_() ->
    ...
    % ?debugFmt("~nin basic_parser_failure_test_~nGot ~ts~nExp ~ts~n", [Got, Exp]),
...
two_line_parser_failure_test_() ->
    ...
    % ?debugFmt("~nin two_line_parser_failure_test_~nGot 1: ~p~n    2: ~ts~nExp 1: ~p~n    2: ~ts~n", [Got1, Got2, Exp1, Err]),
...
pass_through_lexer_parser_failure_test_() ->
    ...
    Exp2 = "Error\n" ++
           ...
           "\n",
    % ?debugFmt("~nin pass_through_lexer_parser_failure_test_~nGot 1: ~p~n    2: ~ts~nExp 1: ~p~n    2: ~ts~n", [Got1, Got2, Exp1, Exp2]),
    ?_assertEqual({Exp1, Exp2}, {Got1, Got2}).
