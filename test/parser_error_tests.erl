...
basic_parser_failure_test_() ->
    ...
    % ?debugFmt("~nin basic_parser_failure_test_~nfrom ~p~nGot ~p~nExp ~p~n", [Str, Got, Exp]),
...
two_line_parser_failure_test_() ->
    ...
    % ?debugFmt("~nin two_line_parser_failure_test_~nfrom ~p~nGot 1: ~p~n    2: ~p~nExp 1: ~p~n    2: ~p~n", [Str, Got1, Got2, Exp1, Exp2]),
...
pass_through_lexer_parser_failure_test_() ->
    ...
    Exp2 = "Error\n" ++
           ...
           "\n",
    % ?debugFmt("~nin pass_through_lexer_parser_failure_test_~nfrom ~p~nGot 1: ~p~n    2: ~p~nExp 1: ~p~n    2: ~p~n", [Str, Got1, Got2, Exp1, Exp2]),
    ?_assertEqual({Exp1, Exp2}, {Got1, Got2}).
